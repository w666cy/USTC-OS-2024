...
size_t heap_size = 0;
// size_t heap_size = 4 * WSIZE;
double get_utilization() {
    return user_malloc_size * 1.0 / heap_size; // 0/0 ?
    // return (double)(user_malloc_size / heap_size); // Why fail?
}
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    if (size - asize < MIN_BLK_SIZE) {
        ...
        // PUT(HDRP(NEXT_BLKP(bp)), PACK(GET_SIZE(HDRP(NEXT_BLKP(bp))), 1, 1)); // the same
        user_malloc_size += size - WSIZE;
    } 
    else {
